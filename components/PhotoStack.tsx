...
function PolaroidCamera({ isDark, scale = 1 }: { isDark: boolean; scale?: number }) {
  ...
  return (
    <div style={{ width: 160*scale, height: 140*scale, borderRadius: 14*scale, background: body, position: "relative", boxShadow: `0 ${8*scale}px ${32*scale}px rgba(0,0,0,0.28)`, border: `${scale}px solid ${subtle}`, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: 20*scale, left: 0, right: 0, height: 4*scale, display: "flex" }}>
        {["#E04B52","#F2B632","#FFE566","#5FE0B3","#3A6EA5"].map((c, i) => <div key={i} style={{ flex: 1, background: c }} />)}
      </div>
...
export default function PhotoStack() {
  ...
  // All cards start in DOM; only opacity is toggled
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (!entry.isIntersecting) return; observer.disconnect(); playSequence(); },
      { threshold: 0.25 }
    );
  ...
  useEffect(() => {
    ...
    const onLeave = () => tiltRefs.current.forEach(el => { if (el) el.style.transform = "perspective(700px) rotateX(0deg) rotateY(0deg)"; });
    ...
    function tick() {
      ...
      if (container) {
        ...
        PHOTOS.forEach((photo, i) => {
          ...
          const dist = Math.sqrt(dx*dx + dy*dy);
          if (dist < 150 && dist > 0) { const f = (1 - dist/150)*15; offs[i].tx = (dx/dist)*f; offs[i].ty = (dy/dist)*f; }
          else { offs[i].tx = 0; offs[i].ty = 0; }
  ...
  // Pure white / dark frame — no semi-transparent values that cause artifacts
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      <AnimatePresence>
        {cameraVisible && (
          <motion.div
            ...
            style={{ position: "absolute", left: "50%", top: "22%", transform: "translate(-50%, -50%)", zIndex: 90 }}
            initial={{ opacity: 0, y: -200 }}
            animate={{ opacity: 1, y: cameraY }}
            exit={{ opacity: 0, y: -280 }}
            transition={cameraY === -280
          ...
          >
            ...
            <AnimatePresence>
              {flashActive && (
                <motion.div
                  style={{ position: "absolute", top: -8*camScale, left: -8*camScale, width: 44*camScale, height: 44*camScale, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,255,220,0.98) 0%, rgba(255,240,120,0.6) 35%, transparent 70%)", pointerEvents: "none" }}
            ...
            <AnimatePresence>
              {blankIdx !== null && (
                <motion.div
                  style={{ position: "absolute", bottom: -(4*camScale), left: "50%", transform: "translateX(-50%)", width: 68*camScale, height: 88*camScale, background: isDark ? "#2A2A2A" : "#FFFFFF", border: isDark ? "2px solid #3A3A3A" : "2px solid #E0E0E0", borderRadius: 2, boxShadow: "0 4px 16px rgba(0,0,0,0.2)", zIndex: 95, pointerEvents: "none" }}
      ...
      {/* Photos — all in DOM from start, image src always set, only opacity animated */}
      {PHOTOS.map((photo, i) => {
        ...
        return (
          <div
            ...
            style={{ position: "absolute", top: `${photo.top}%`, left: `${photo.left}%`, zIndex: isHovered ? 999 : 10 }}
          >
            <div
            ...
            >
              <motion.div
                initial={{ opacity: 0, rotate: photo.rotate, scale: 0.92 }}
                animate={
                  visiblePhotos[i]
                    ? { opacity: otherHovered ? 0.7 : 1, rotate: photo.rotate, scale: 1 }
                    : { opacity: 0, rotate: photo.rotate, scale: 0.92 }
                }
                transition={
                  visiblePhotos[i]
                ...
                whileHover={sequenceDone ? { y: -12, scale: 1.4, transition: { type: "spring", stiffness: 300, damping: 20 } } : {}}
                onHoverStart={() => sequenceDone && setHoveredIdx(i)}
                onHoverEnd={() => sequenceDone && setHoveredIdx(null)}
                style={{ cursor: sequenceDone ? "pointer" : "default" }}
              >
                {/* Hue glow on hover */}
                {isHovered && (
                  <motion.div
                    style={{ position: "absolute", inset: -8, borderRadius: 16, background: hueColor, opacity: 0.35, filter: "blur(20px)", zIndex: -1, pointerEvents: "none" }}
                    initial={{ opacity: 0 }}
                ...
                <div style={{
                  ...
                  <div style={{ width: "100%", aspectRatio: "1 / 1", overflow: "hidden", borderRadius: 1, background: isDark ? "#1A1A1A" : "#E8E8E8" }}>
                    <img
                      src={photo.image}
                      alt={photo.label}
                      loading="eager"
                  ...
                  <p style={{ fontFamily: "'Caveat', cursive", fontSize: 12, color: labelColor, textAlign: "center", margin: "6px 0 2px", lineHeight: 1 }}>
                    {photo.label}
                  </p>
                </div>
              </motion.div>
